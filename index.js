...
client.on("message", async message => {
    ...
    if (message.content === `${config.PREFIX}haya`) {

        var hayamsg = new Discord.MessageEmbed()
            .setTitle(data.haya.title)
            .setDescription(data.haya.des)
        client.on('messageReactionAdd', async (reaction, user) => {
            if (reaction.emoji.id === '875410119217799239') {
                if (user.id === message.author.id) {
                    var notu = new Discord.MessageEmbed()
                        .setTitle(data.haya.u)
                        .setColor('RED')
                    return message.inlineReply(notu);
                }
                if (user.bot === true) {
                    return;
                } else {
                    var responehayamsg = new Discord.MessageEmbed()
                        .setTitle(data.haya.respone)
                        .setColor('GREEN')
                    message.channel.send('<@' + user.id + '> بیااااااااااا که جوابت رو داد <@' + message.author.id + '>', { embed: responehayamsg })
                }
            }
        });
        message.inlineReply(hayamsg).then(function (message) {
            message.react('<:585669781567307803:875410119217799239>')
        })
    }
    ...
    if (profilecommand === 'profile') {
        if (!profileargs.length) {
            var profilehelp = new Discord.MessageEmbed()
                ...
                .setDescription(`<a:cameraa:875411271250481233> **Profile Commands:**\n<:space:874678195843125278><:e1:875415555841077268> ${prefix}egirl\n<:space:874678195843125278><:e2:875416681474850866> ${prefix}eboy\n<:space:874678195843125278><a:couplee:875418191067775047> ${prefix}couple\n<:space:874678195843125278><a:landscape:875418222558597230> ${prefix}landscape\n<:space:874678195843125278><:animee:875419748542869624> ${prefix}anime\n<:space:874678195843125278><a:boyy:875419154524569630> ${prefix}boy\n<:space:874678195843125278><a:girll:875420525751582781> ${prefix}girl\n<:space:874678195843125278><:animal:875420910776090675> ${prefix}animal`);
